...
import style from './AddFolderButton.module.scss'

const AddFolderButton = ({ colors, addFolder, isFetching, subscibe }) => {
    ...
    const onChangeColor = colorId => setColor(colorId);
    const onChangeInput = value => setInputValue(value);

    const onClickPopup = event => {
        console.log(!!event.target.closest('.addList__popup'));
    }
    ...
    const onClickOut = event => {
        if(editMode && !event.target.closest('.addList__popup')){
            closePopup()
        };
    };
        ...
        <div className={style.addList}>
            ...
            {editMode && (
                ...
                >
                    ...
                    <ul>
                        {colors.map(color => (
                            <li key={color.hex} className={style.addList__colors} >
                                <i
                                    className={classnames(
                                        style.addList__color,
                                        { [style.active]: color.id === selectedColor }
                                    )}
                                    style={{
                    ...
                    >{isFetching ? 'Добавление...' : 'Добавить'}</button>
                    <div
                        className={style.addList__close_icon}
                        onClick={closePopup}
                    >
